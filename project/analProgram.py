...
# For now going to assume it's always an expression statement
# when IfStatement, WhileStatements are involved this may need to adapt
def checkSteps(programSteps, result):
    for step in programSteps:
        assignmentType = step["type"]
        assignmentTypes[assignmentType](step, result)
